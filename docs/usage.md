Once you've copied the flake/npins example, it's time to start defining some wrapprs. To do this, we'll use a strategy
...
let
  ...
  root = {
    modules = lib.recursiveUpdate adios-wrappers (adios.lib.recursiveUpdate ./wrappers);
  };
```
